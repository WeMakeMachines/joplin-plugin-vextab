...
- `/src/joplin-plugin-vextab.ts`, which contains the entry point for the plugin source code.
...
To get such an external script file to compile, you need to add it to the `extraScripts` array in `plugin.config.json`. The path you add should be relative to /src. For example, if you have a file in "/src/webviews/joplin-plugin-vextab.ts", the path should be set to "webviews/joplin-plugin-vextab.ts". Once compiled, the file will always be named with a .js extension. So you will get "webviews/joplin-plugin-vextab.ts" in the plugin package, and that's the path you should use to reference the file.
